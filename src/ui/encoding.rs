...
impl State {
    ...
    pub fn start(input: PathBuf, output: PathBuf, cutoff: f32, encoder: Codec, backend: ComputeBackend, dct_algorithm: DctAlgorithm) -> (Self, Task<Message>) {
        ...
        std::thread::spawn(move || pipeline::run(&input, &output, cutoff, encoder, backend, dct_algorithm, MediaBackendChoice::ALL[0], tx));
